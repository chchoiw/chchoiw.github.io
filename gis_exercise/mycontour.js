...
L.CanvasLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  onAdd: function onAdd(map) {
    ...
    // map.removeLayer(this._stationLayer);

    // this._stationLayer= this.addStation();
  ...
  addTo: function addTo(map) {
    map.addLayer(this);
    // map.addLayer(this._stationLayer);
    return this;
  },
...
L.Control.Contour= L.Control.extend({
  ...
  _onMouseMove: function _onMouseMove(e) {
    ...
    if (gridValue && this.options.dataKey !="WSPD_10")
    {
       htmlOut = "<strong> ".concat(this.options.dataKey," ").concat((gridValue).toFixed(2)).concat(this.options.speedUnit);

    }
    else if (gridValue && this.options.dataKey =="WSPD_10")
    {
       htmlOut = "<strong> ".concat(this.options.dataKey," ").concat((gridValue*3.6).toFixed(2)).concat(this.options.speedUnit);
...
L.ContourLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  // timeSeries:,
_addStation: function addStation() {
  var path = this.options["displayOptions"]["stationGeoPath"];
  var colorLevel = this.options["colorScale"];
  var dataKey = this.options["displayOptions"]["dataKey"];
  ...
  for (var i = 0; i < stationData.length; i++) {
    ...
    if (typeof nameMarker != "undefined" && nameMarker != null) {
      ...
      var htmlstr =
        ...
        "_" +
        dataKey +
        "_spanValue' class='dataSpan' style='background-color:" +
        speedcolor +
        ";'>" +
        zScale +
        "</div>";
      ...
      var popStr =
        ...
        " " +
        zScale +
        this.options.displayOptions.speedUnit +
        "</div><button id='showChartBtn' station='" +
        name +
        "' varName='" +
        dataKey +
        "'  onclick =\"appendData('" +
      ...
      var stationMarker = L.marker([lat, long], {
        icon: new L.DivIcon({
          className: "waterstation-icon " + idstr,

          html: htmlstr,
        }),
      });
      if (zScale != "NaN") {
        stationMarker.bindPopup(popStr, customOptions);
      }
      ...
      stationMarker.on("click", function (ev) {
        ...
        var myCustomPopup = stationMarker.getPopup().getElement().children[0]
        ...
        stationMarker.getPopup().getElement().children[0].style.zIndex = 600;
      });
  ...
  onAdd: function onAdd(map) {
    ...
    var dataKey = this.options.displayOptions.dataKey;
    try {
      var lastVarName = ctx.getAttribute("lastVarName");
    } catch (error) {
      console.error(error);
      var lastVarName = "";
    }

    if (dataKey != lastVarName) {
      ctx.width = ctx.width;
      ctx.getContext("2d").clearRect(0, 0, 3000, 3000);
    }
    if (map.getPane) {
    ...
    this._stationLayer = this._addStation();

    this._canvasLayer = L.canvasLayer({
      pane: pane,
    }).delegate(this);
    this._stationLayer.addTo(map);
  ...
  setOptions: function setOptions(options) {
    ...
    if (options.hasOwnProperty("dataKey"))
      this.options.dataKey = options.dataKey;
  ...
  _startWindy: function _startWindy() {
    ...
    console.log("b,s",bounds,size);
    // console.log("startWindy",bounds._southWest.lng,size.y)
    this._windy.start(
      [
        [0, 0],
        [size.x, size.y],
  ...
  _initWindy: function _initWindy(self) {
    ...
    var canvasLayer = self._canvasLayer._canvas;

    console.log("canvase", canvasLayer);
    ...
    // this._canvasLayer._canvas.id = "canvas1";
    this._canvasLayer._canvas.classList.add("contour-overlay");
    var lastVarName = this._canvasLayer._canvas.getAttribute("varName");
    this._canvasLayer._canvas.setAttribute("lastVarName", lastVarName);
    this._canvasLayer._canvas.setAttribute(
      "lastVarName",
      this.options.displayOptions.dataKey
    );
    // this._canvasContourLayer._canvas.classList.add("contour-overlay");
    this.onDrawLayer();
  ...
  _initMouseHandler: function _initMouseHandler(voidPrevious) {
    ...
    if (!this._mouseControl && this.options.displayValues) {
      ...
      this._mouseControl = L.control.contour(options).addTo(this._map);
    }
  },
  _clearAndRestart: function _clearAndRestart() {
    // if (document.getElementById("canvas1").getContext("2D"))
    // document.getElementById("canvas1").getContext("2D").clearRect(0,0,3000,3000);
    console.log("check clear and Restart")
    if (this._context) {
...
var Windy = function Windy(params) {
    ...
    // console.log("g.length",typeof(g00))
    ...
    var createWindBuilder = function createWindBuilder(uComp,header) {
      var uData = uComp;
        //   vData = vComp;
      return {
        header: header,
    ...
    //   });
      ...
      return createWindBuilder(data["z"],data["header"]);
    };
      ...
      if (row = grid[fj]) {
        ...
        if (isValue(g00) && isValue(g10) && (row = grid[cj])) {
          ...
          if (isValue(g01) && isValue(g11)) {
            ...
            // console.log("test")
            var tmp=builder.interpolate(i - fi, j - fj, g00, g10, g01, g11);
            // console.log("bilinear",tmp)
    ...
    //   wind[0] = d[0] * u + d[2] * v;
    //   wind[1] = d[1] * u + d[3] * v;
    wind=Math.sqrt( ((d[0] +d[2])*(d[0] +d[2]) +(d[1] +d[3])*(d[1] +d[3]))* u*u);
    // console.log("distort",wind,u)
      return wind;
    };
    ...
    function drawBoundary(grid, bounds) {
      ...
      g3.clearRect(0, 0, 3000, 3000);
      console.log("draw boundary again")
      var bbox = {
        type: "Polygon",
        coordinates: [
          [
      ...
      var projectionMer = d3
        .geoMercator()
        .fitSize([params.canvas.width, params.canvas.height], bbox);

      var path = d3.geoPath().projection(projectionMer).context(g3);
      path(topojson.mesh(macauGeoData));
      g3.strokeStyle = asColorStyle(255, 0, 0, 1);
      g3.lineWidth = 2;
      g3.stroke();
      g3.fillStyle = asColorStyle(255, 255, 0, 1);
      g3.fill();
      g3.strokeStyle = asColorStyle(255, 255, 0, 1);
      g3.lineWidth = 2;
      g3.stroke();
      imageData = null;
      imageData = g3.getImageData(
        0,
        0,
        params.canvas.width,
        params.canvas.height
      ).data;
      // console.log(params.canvas.width);
    }
    ...
    function drawContour(grid,bounds)
    {
      ...
      function drawColumn(x,isLogarithmic) {
        // var x = bounds.x;
        var xBound = bounds.x + bounds.width; // upper bound (exclusive)
        var yBound = bounds.y + bounds.height; // upper bound (exclusive)
        var min,max;
        // console.log("grid2",grid.data,typeof(grid.data))
        grid.data.forEach(function(itm) {
          // console.log("fixbug",item)
        ...
        var range=max-min
        for (var y = bounds.y; y < yBound; y += 20) {
          if (displayMask(x, y, imageData, params.canvas.width)) {
            var coord = invert(x, y);
            if (coord) {
              var λ = coord[0],
                φ = coord[1];
              if (isFinite(λ)) var z = interpolate(λ, φ);
              var zScale;
              var dataKey = params.displayOptions.dataKey;
              if (dataKey == "WSPD_10") {
                zScale = z * 3.6;
                // zScale=z;
              } else {
              ...
              var colorLevel = params.colorScale;

              var color;
              var leng = colorLevel.length;
              for (var i = 0; i < leng - 1; i++) {
                if (
                  zScale >= colorLevel[i][0] &&
                  zScale < colorLevel[i + 1][0]
                ) {
                  var red = colorLevel[i][1][0];
                  var green = colorLevel[i][1][1];
                  var blue = colorLevel[i][1][2];
                  var opacityColor = colorLevel[i][1][3];
                  color = asColorStyle(red, green, blue, 0.6);
                  break;
                } else color = asColorStyle(0, 0, 0, 0);
              }
              g3.fillStyle = color; //asRainbowColorStyle(0,z2,0, 0.4);
              g3.fillRect(x, y, 20, 20);
            }
          }
        }
      }    
  ...
  return !pixelBuffer.some(color => color !== 0);
    ...
    function drawLegend(bounds)
    {


              var colorLevel=params.colorScale;
              var ctxindex=document.getElementById("myCanvas");
              
              var g=ctxindex.getContext("2d");
              // var leng=Math.floor(bounds.width/colorLevel.length);
              // var lastVarName=ctxindex.getAttribute("varName");
              var dataKey=params.displayOptions.dataKey;
              //#####################
              // draw legend
              //###################
            ...
            var lastVarName = ctxindex.getAttribute("varName");             
              // if(true)
              // console.log(lastVarName,dataKey,isCanvasBlank(ctxindex),lastVarName!=dataKey )
              if (isCanvasBlank(ctxindex) || lastVarName!=dataKey)
              {
                ctxindex.width=ctxindex.width;
                // console.log(lastVarName,dataKey)
                ctxindex.setAttribute("varName", dataKey);
                ctxindex.setAttribute("lastVarName", lastVarName);
                var leng=colorLevel.length;
                var n = Math.floor(ctxindex.height/ leng);
                for (var i=0; i<leng-1;i++)
                {
                  
                  // console.log("color",colorLevel[i][0],colorLevel[i][1])
                  var red=colorLevel[i][1][0];
                  var green=colorLevel[i][1][1];
                  var blue=colorLevel[i][1][2];
                  var opacityColor=colorLevel[i][1][3];
                  g.fillStyle = asColorStyle(red,green,blue, opacityColor);
                  // console.log(asColorStyle(red,green,blue, opacityColor))
                  g.fillRect(0, i * n, ctxindex.width, (i + 1) * n);
                  g.font = "18px Arial";
                  g.fillStyle="#fff";
                  g.fillText(
                    colorLevel[i][0].toFixed(0).toString(),
                    ctxindex.width/4,
                    (i + 0.5) * n
                  );
                // console.log(Math.round((min+range/leng*i),2).toString()+'m/s')
                // g.fillText('m/s',i*(n),30);
                }
                g.fillStyle="#fff";
                g.fillText(
                  params.displayOptions.speedUnit,
                  ctxindex.width / 4,
                  (leng - 0.5) * n
                );
              }
