...
function initDemoMap(){
    ...
    var Esri_WorldImagery = L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager_nolabels/{z}/{x}/{y}{r}.png', {
...
var coorsLayer = L.geoJSON(stationData, {
    ...
    pointToLayer(feature, latlng) {
      ...
      {
        var htmlstr='<span class="my-div-span">'+feature.properties.name+'</span><br>'+"<span class='dataSpan'>"+(feature.properties.WSPD_10*3.6).toFixed(1)+"</span>"
      }
      else
      {
        var htmlstr='<span class="my-div-span">'+feature.properties.name+'</span><br>'+"<span class='dataSpan'>NaN</span>"        
      }
...
//     }
//   });
    var speedColorScale=[[0,[98,113,183,255]],
                  [1,[57,97,159,255]],
                  [3,[74,148,169,255]],
                  [5,[77,141,123,255]],
                  [7,[83,165,83,255]],
                  ...
                  [77,[205,202,112,256]],
                  [104,[128,128,128,255]]];
      var tempColorScale=[[0 ,[93,133,198,255]],
                  [1 ,[68,125,99,255]],
                  [10 ,[128,147,24,255]],
                  [21 ,[243,183,4,255]],
                  [30 ,[232,83,25,255]],
                  [47 ,[71,14,0,255]]];
      var humiColorScale=[[0,[173,85,56,255]],
              [30,[173,110,56,255]],
              [40,[173,146,56,255]],
              [50,[105,173,56,255]],
              [60,[56,173,121,255]],
              ...
              [97,[56,98,157,255]],
              [100,[56,70,114,255]]];
      var wlColorScale=[[0,[159,185,191,255]],
                  [0.5,[48,157,185,255]],
                  [1,[48,98,141,255]],
                  [1.5,[56,104,191,255]],
                  [2,[57,60,142,255]],
  ...
  // function addTotalSation(filename,key,fileTimeFormat="")
  if (fileTimeFormat!="")
    var windbarbMarkerList=windbarbGen("./awsData/stationPoint/windbarb_"+fileTimeFormat+".json",0)
  else
    ...
    // d3.csv(filename).then((data) => {
        ...
        focus
          ...
          .attr("class", "y3")
            ...
            .attr('stroke-width', '3')
            .style("opacity", "0.7");
        focus.append("text").attr("class", "y1").attr("dx", 8)
        .attr("dy", "-.3em");
        focus.append("text").attr("class", "y2").attr("dx", 8)
        .attr("dy", "-.3em");
        focus.append("text").attr("class", "y3").attr("dx", 8)
        .attr("dy", "1em");
        focus.append("text").attr("class", "y4").attr("dx", 8)
        .attr("dy", "1em");
        ...
        function mouseMove(event,action="mousemove") {
            ...
            if (window.mobileAndTabletCheck())
            {
              var width2 = document.getElementById(chartID)
    ...
    WindArrow: function(speed, direction, container,arrowWidth, x,y) {
      ...
      if (this.speed > 0) {
        ...
        // ));
          ...
          'd': this.path,
          'vector-effect': 'non-scaling-stroke',
          'transform': 'translate(' +x + ', ' + y + ') scale(' + this.scale + ') rotate(' + this.trigDirection + ' ' + 0 + ' ' + 0 + ')  translate(-8, -2)',
          'class': 'wind-arrow',
