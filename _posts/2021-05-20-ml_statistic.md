...
卜瓦松分布的母數$\lambda$是單位時間（或單位面積）內隨機事件的平均發生率。
...
- 连续变量的缺失值占比在85%左右时，根据是否缺失，生成指示变量，仅使用指示变量作为解释变量
...
## 對應分析
- **对应分析**用于两个**离散型变量**之间的分析
- 	能够分析变量（列）与样本（行）之间的关系
- 够分析样本（行）与样本（行）之间的关系## 時間序列
- 	两个向量的长度越长，且夹角越小，那么对应性越强
