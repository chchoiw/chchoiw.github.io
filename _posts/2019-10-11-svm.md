...
$$
\begin{align}
\frac{\partial \mathcal{L}(w,b,\alpha)}{\partial w}&=w-A^TD\alpha=0 \implies w=A^TD\alpha   \label{org_tran4}\\
\frac{\partial \mathcal{L}(w,b,\alpha)}{\partial b}&=\alpha D\mathbf 1 \implies \sum\limits_{i=1}^l \alpha_i y_i
=0 \label{org_tran5}
\end{align}
$$

Subtitute $\eqref{org_tran1}$ into $\eqref{original}$,$\label{org_tran2}$ we have
...
Subtitute a into $\mathcal{L}(w,b,\xi,\alpha,\gamma)\$, we have
