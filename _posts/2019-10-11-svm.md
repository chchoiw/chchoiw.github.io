...
$$
\begin{aligned}
\frac{\partial \mathcal{L}(w,b,\alpha)}{\partial w}&=w-A^TD\alpha=0 \implies w=A^TD\alpha  \\
\frac{\partial \mathcal{L}(w,b,\alpha)}{\partial b}&=\alpha D\mathbf 1 \implies \sum_{i=1}^l \alpha_i y_i
=0 
\end{aligned}\label{transform}
$$

Subtitute $\eqref{original}$ into $\eqref{transform}$, we have 
...
$$
\max_{x\in\mathbb R^l} -\frac{1}{2}\alpha^TDAA^TD\alpha+\alpha^T 
$$
...
$$
\min_{w,b,\xi \in \mathbb R^{n+1+l}} \frac{1}{2}\Vert w\Vert_2^2+\frac{C}{2}\Vert \xi\Vert_2^2
$$
...
$$
\min_{w,b,\xi \in \mathbb R^{n+1+l}} \frac{1}{2}\Vert w\Vert_2^2+C\mathbf 1^T\xi
$$
...
=0
\end{align}\label{eq:partial_eqs}
$$

Subtitute $\eqref{eq:partial_eqs}$ into $\mathcal{L}(w,b,\xi,\alpha,\gamma)$, we have
...
$$
\max_{\alpha\in \mathbb R^l}\mathbf 1^T \alpha-\frac{1}{2}\alpha^TDAA^TD\alpha
$$
