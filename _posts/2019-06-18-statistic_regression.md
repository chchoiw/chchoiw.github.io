...
# 回歸分析的步驟
- [ref](https://mengte.online/archives/1899)
- 從樣本數據出發，確定變量之間的數學關系式：
- 估計回歸模型參數；
- 對所確定的關系式進行各種統計檢驗，並從影響某一特定變量的諸多變量中找出影響顯著的變量。
- 對回歸模型進行評價與檢驗。對回歸模型進行評價與檢驗的目的主要是研判回歸方程的可靠性，它包括經濟意義檢驗、擬合優度、顯著性檢驗、D-W檢驗、 方差膨脹因子檢驗等。
- 根據回歸模型進行預測。如果回歸模型通過評價及相關檢驗，則可利用回歸模型進行預測。

## 模型參數估計
- 回歸統計： 相關系數、判定系數，標準誤和觀察個數
  - 擬合程度$R^2$
- 第二部分是方差分析，這部分給出的是回歸方程的方差分析表。
- 第三部分是模型中參數估計的有關內容。

## 模型一些已知假設
- 多重共線性:
  - 檢測：VIF
  - 改善：一是使用逐步回归分析；二是使用岭回归分析，三是进行相关分析，手工移出相关性非常高的分析项，然后再做线性回归分析
- 自相關:D-W值常用于检验序列一阶自相关
- 異方差