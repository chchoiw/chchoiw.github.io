...
## 從焦點和距離差到一般式
因為題目已知是焦點和曲線上到兩焦點絕對距離差,但簡化的兩條曲線$\eqref{H1},eqref{H2}$都是一般式$A^2+Bxy+Cy^2+Dx+Ey+F=0$,所以先從已知焦點和曲線上到兩焦點絕對距離差求解出一般式的關係
...
現在已經很接近$\eqref{H1},eqref{H2}$,但$\eqref{H1}$是更簡化的情況, 所以考慮進一步變化雙曲線$H_1$<br>
...
聯立以上兩條方程會得出 $k_4y^4+k_3y^3+k_2y^2+k_1y+k_0=0$, 只需要求出$k_4,k_3,k_2,k_1,k_0$就能透過一元四次求解公式得出交點
