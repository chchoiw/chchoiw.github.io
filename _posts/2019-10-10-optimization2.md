...
$$
\min\limits\limits_{x\in \Omega}f(x) \quad \text{s.t.} \quad g(x) \leq 0 \quad \text{and} \quad h(x)=0
$$
...
$$
\frac{\partial \mathcal L (x,\alpha,\beta)}{\partial x} \bigg\vert\limits_{x=\bar x} =\nabla f(\bar x)+\bar \alpha^T \nabla g(\bar x)+\bar \beta^T \nabla h(\bar x) =0
$$
...
$$
\begin{aligned}
&\min\limits\limits_{x \in \mathbb R^n} p^Tx \\
...
$$
\begin{aligned}
&\max\limits\limits_{\alpha \in \mathbb R^m} b^T\alpha \\
