...
title: '02333-軟件工程'
date: 2022-07-04
permalink: /posts/2022/07/software/
tags:
...
- 軟件測試過程模型 
  - 軟件測試是一個有程序的過程，包括測試*設計*．測試*執行*、測試*結果比較*等。
- 軟件測試的首要目標是*預防錯誤*
- 一個成功的測試可以*發現尚未發現的錯誤*
...
- 白盒測試技術，又稱為*結構測試技術，*典型的是*路徑測試技術*。
    ...
    |分類|典型|依據|
    |----|----|----|
    |白盒測試技術<br>（又稱結構測試技術）|路徑測試技術|程序的邏輯結構|
    |黑盒測試技術<br>（又稱功能測試技術）|事務處理流程技術<br>狀態測試技術<br>定義域測試技術|軟件行為的描述|
...
- 集成測試
  ...
  - 驅動模塊:主控模塊，設計承接模塊替代其直接的下屬模塊，按選取的測試方式（先深度/先寬度），在*組合模塊時進行測試*。
- 有效性測試
  - 技術：黑盒測試技術。
  - 驗證：軟件需求的可追潮性。
  - 任務：關注檢驗是否符合用戶的文檔。
...
| 功能|驗證|確認|
|----|----|----|
|定義|證實產品是否正確反映規約的需求|證實*使用的軟件*是否滿足需求|
|作用|證實是否正確的反映了所規約的需求|證實所期望使用的軟件工作產品是否滿足其需求|
|區別|與該產品所要求的*特征*進行比較|反映特定期望使用的*特殊需求*|

- 簡述驗證和確認的定義、作用和區別。
  - 驗證是通過提供的客觀證據，證實規約的需求是否得以滿足。（1分）
  - 確認是通過提供的客觀證據，證實有關特定期望的使用或應用需求是否得以滿足。（1分）
  - 確認是通過提供的客觀證據，證實有關特定期望的使用或應用需求是否得以滿足。（1分）
  - 確認的作用是證實所期望使用的軟件工作產品是否滿足其需求。（1分）
  - 驗正是把在生存期上下文中的一個產品與該產品所要求的特征進行比較的活動。（0.5分）
  - 確認是反映特定期望使用的特殊需求，是否滿足其期望的使用。（0.5分）


### 瀑布模型
- 概念：將軟件生存周期的各項活動規定為按*固定順序*而連接的若干階段工作，形如瀑布流水，最終得到軟件產品：
- 作用：*支持*結構化軟件開發、*控制*軟件開發的複性、*促進*軟件開發工程化。
...
### 增量模型
- 概念：指*需求*可以分組，形成一個個的*增量*，並可形成一個*結構*，即該模型有一個前提，需求可結構化，在此條件下，對每一增量實施*瀑布式開發*。
- 適用："技術驅動"的軟件產品開發，常被工業界所采用
- 優點
  - 第一個可交付版本需時間和成本較少
  - 減少用戶需求的變更
  - 增雖投資
...
- 概念
  - 基礎：用戶提出待開發系統的核心需求
  - 過程
    - 根據需求，開發一個核心系統並投入運行，以便用戶能夠有效提出反饋
    - 軟件開發人員根據用戶反饋；
    - 實施開友時送代過程。
- 迭代：由需求、設計、編碼、測試、集成組成。
- 適用：事先不能完整定Y需求的軟件

### 螺旋模型
- 概念：
  - 在瀑布模型和演化模型的基礎上，加入兩者忽略的風險分析；
  - 沿看螺旋線，經歷制定計劃，風險分析，實施工程客戶評估這4個方面的活動；
  - 自內向外每旋轉一圈便產生一個更為完善的新版本。
- 適用：項目的*開發風險很大*或客戶不能確定系統需求。

### 噴泉模型
- 特征：迭代、*無間隙*
- 意義：說明了軟件活動需要多次重復；活動之問沒有明顯的間隙。
- 適用：面向*對象技術*的軟件開發。


- 瀑布模型規定了各開發階段的活動，並且自上而下具有相互街接的*固定*順序，還規定了每一階段的*輸人*作為輸出傳送到下一階段。
- 瀑布模型規定了各開發階段的活動：*系統需求*、*軟件需求*、*需求分析*、*設計*、 *編碼*，*測試*和*運行*。
- 螺旋模型是由*瀑布*模型和*演化*模型結合而建立的一種軟件開發模型。
- 常用的軟件生存周期模型中，主要用於支持面向對象軟件開發的模型是*噴泉模型*
- 軟件開發的螺旋模型適用*客戶不能確定系統需求的項目*
- 以下哪個軟件生存周期模型是一種風險驅動的模型*螺旋模型*
- 軟件開發的增量模型是一種*快諫構造產品第一版本的好方法*
- 演化模型*不*具有的特性是*有效風險控制機制*
...
- 選選軟件生存周期模型(SLCM)附步驟
  ...
  - *標識*為選擇SLCM所需要的任何約束
  - *評估*所選擇的SLCM
  - *選擇*最能滿足項目屬性和約束的 SLCM

- 關於軟件生存周期過程的監控
  - 軟件生存周期過程、軟件生存周期模型、軟件項目過程管埋的天系
    - *軟件生存周期過程*，回答軟件開發需要做哪些工作。
    - *軟件生存周期模型* ，回答軟件開發活動或任務如何組織。
    - *軟件項目過程管理*，回答軟件過程如何管理。
  - 軟件生存周期過程是軟件生存周期模型和軟件項目過程管理的*基礎*。
  - 軟件生存周期模型為軟件項目過程管理提供*支持*。


- 對於一個項目而言，過程管理計劃是項目管理計劃的主體，一般還可能存在一些對支持生存周期過程具有重要作用的其他計劃，包括*軟件工程管理計劃、軟件配置管理計劃、軟件質量保證計劃、軟件驗證和確認計劃和軟件度量計劃*
- 過程管理包括*過程建立、過程評估和過程改進*
...
```
- CMMI提出所基於的基本思想
  - CMMI模型基於*過程途徑*思想，把軟件質量的 3個支撐點：*受訓的人員、規程和方法、工具和設備*進行集成，以開發所期望的系統產品。
  - CMMI緊緊圍練*開發、維護和運行*，把經過證明的最佳實踐放在一個結構中。
  - 該結構*有助於*指導組織確定其過程的改善優先次序；有助於指導這些改善的實施，以提高其過程能力和成熟度，還*支持*其他領域能力成熟度模型的開發。


- CMMI是針對系統/產品開發的能力成熟度模型，集成了3個源模型：*軟件CMM、 系統工程CMM和產品集成開發CMM*
- CMMI模型基於*過程途徑*思想，把軟件質量的 3個支撐點：*受訓的人員、規程和方法、工具和設備*進行集成，以開發所期望的系統產品。

- 簡述CMMI提出所基於的基本思想。
  - 基於過程途徑用想，通過過程把軟件質量的3個支撐點：受訓的人員、規程和方法、工具和設備進行集成。（2分）
  - CMMI緊緊圍繞開發、維護和運行，把經過證明的最佳實踐放在一個結構中。（1分）
  - 該結構有助於指導組織確定其過程的改善優先次序；（1分）
  - 有助於指導這些改善的實施，以提高其過程能力和成熟度，並且還支持其他領域能力成熟度模型的開發。（1分）
...
### CMMI 能力等級
- 為改善其開發過程和維護過程的組織，CMMI提供了2種類型的等級這2種等級描述了2種過程改善的演化路徑：*能力等級、成熟度等級*。
- 過程能力：是指遵循一個過程可達到的預期結果的程度。
- 能力等級：一種*過程改善路徑*，該路徑可使組織針對*單一*過程域不斷改善該過程域，即指在單一過程域中己達到的過程改善。能力等級是為了管理，對過程改善程度所設定的幾個”台階”。
- CMMI有6個共用目標，用於表征*過程制度化*的程度，能萬等級1~5分別對應共用目標*1~5*的過程代表著過程制度化程度的提高。故共用目標編號越小，制度化程度越低。
...
###  組織成熟度等級
- 成熟度等級：一種*過程改善路徑*，該路徑可使組織通過關注*一組*過程域不斷改善一組相關過程域：意在改進組織的整體性能。
- 成熟度等級的組成：由預先定義的*一組過程域集*及其相關的一些*專用實踐和共用實踐*。
- *能力等級*和*成熟度等級可*用於評定活動、估算，作為過程評估的結果。
...
- CMMI的能力等級和成熟度等級在概念上是互補的，區別在於能力等級是用來表征組織對*單個*過程域的改善。
- 在CMM模型中，遵循一個過程可達到的預期結果的程度是*過程能力*
- CMMI模型提供了兩種過程改善路徑，一是稱為*能力等級*的過程改善路徑，二是稱為*成熟度等級*的過程改善路徑。
- CMMI有6個共用目標，用於表征過程制度化的程度。目標編號越小，說明該過程的制度化程度就越*低*
- 簡述CMMI成熟度等級的概念、劃分和組成。
  - 成熟度等級是指達到預先定義的*一組過程域所有目標的一種過程改善等級*，意在改進組織的整體性能；（1分）
  - 1級一初始級、2級一已管理級、3級一已定義級、4級一已定量管理級、5級一持續優化級
  - 成熟度等級是由*預先定義*的*一組過程域集*及其相關的一些*專用實踐*和*共用實踐組成的*
- 簡述CMMI模型支持的兩種過程改善路徑。
  - 為了改善其*開發過程和維護過程的組織*，CMMI提供了兩種類型的等級
  - 能力等級是一種*過程改善路徑*，該路徑可使組織針對單一過程域不斷改善該過程域：
  - 成熟度等級也是一種過程改善路徑，該路徑可使組織通過關注一組過程域不斷改善一組相關的過程域；（1分）
  - 這兩種等級還可用於評定活動、估算，作為過程評估的結果；
  - 能力等級和成熟度等級，這兩種等級描述了*兩種過程改善的演化路徑*。（1分)
