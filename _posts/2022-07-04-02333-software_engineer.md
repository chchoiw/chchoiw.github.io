...
|用況模型|分析模型|
|----|----|
|客戶和開發者間關於 “系統應做什麼、不應做甚麽"的現約|開發者理解系統如何勾固、如何設計、如何實現的基礎|
...
- 设计模型、部署模型及相关视角下的体系结构描述
  - 设计模型及其视角下的体系结构描述
    - RUP设计主要结果是系统设计模型，它尽量保持该系统具有分析模型结构，并作为系统实现的输入。设计模型包括以下元素：
      - *没计子系统和服务子系统*，及其依赖、接口和内容， 
...
- 設計的主要活動
  ...
  - 活动4：子系统设计
    ...
    - 确保子系统提供正确接口：
    - 确保子系统实现了目标入即给出该子系统提供的接口定义的操作的细化

| 分析模型|设计模型|
|----|----|
|概念模型，对系统的抽象，无实现细节|软件模型，对系统的抽象，无实现细节|
|可用于不同设计|特定于一个实现|
|用3个衍型类：控制类、实体类、边界类|用多个行型类，依赖于实现语言|
|几乎非形式化|较形式化|
...
|概括地给出了系统设计，包括系统的体系结构|表明了系统设计，包括设计视角下的系统(体系结构|
