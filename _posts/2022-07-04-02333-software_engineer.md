...
- 測試策略
  ...
  - 分支覆盖
    - 概念：至少将程序中的每一分支执行一次。
    - 特点：度品强于语句覆盖：不能李出全部错误。 
  - 条件覆盖的概念：
    - 一條路徑中，每个判定中为真的条件取值至少执行一次。
  - 条件组合爱盖的概念：
    - 每个判定中所有可能的条件取值组合至少执行一次。
  - 四种测试策略存在以下基本关系：
    - 语句覆盖<=分支覆盖<=条件组合覆盖<=路径覆盖
  - 在路径测试技术中（*路径选取*是测试用例设计的基础，*好的用例设*計是发现程序错误的关键。
...
- 白盒测试技术
  ...
  - 典型技术:路径测试技术
- 基于事务流的测试技术
  - 事务：指从系统用户角度出发所见到的一个工作单元，由*一系列操作*组成。
  - 事务流程图：一种表达被测软件模型的工具。
- 其他功能测试技术简述
  - 等价类：把软件所有可能输入数据划分成若干部分，一个部分中各数据发现软件错误的概率一样。
  - 有效等价类：合理、有意义的输入数据集合。
  - 无效等价类：不合理、无意义的输入数据集合。

| 情况|划分原则|
|----|----|
|输入条件规定了输入数据取值范围|1个有效等价类、2个无效等价类|
|输入条件规定了输入数据的「数效|1个有效等价类、2个无效等价类|
|输入条件规定了输入数据的一组可能取值|每一个输入值确定1个有效等价类， 针对这组值确定1个无效等价类|
|输入条件是一个*布尔量*|1个有效等价类，1个无效等价类|
|输入条件规定了*必须符合的条件*|1个有效等价卖 1个无效等价类|
|已划分的某一等价类中各元素在程序中的*处理方式不同*|将此等价类进一步划分为更小的等价类|
|输入条件规定了输入数据*取值范围*|1个有效等价类、2个无效等价类|
|输入条件规定了输入数据的*个数*|1个有效等价类、2个无效等价类|
|输入条件规定了输入数据的*一组可能取值*|每一个输入值确定1个有效等价类， 针对这组值确定1个无效等价类|

- 边界值分析
  - 概念：使用等干、小于或大干边界值的数据对软件进行测试，*发现错误的概率较大*。故在设计测试用例时应选择一些边界值。
  - 使用原则
    |情況|测试数据|
    |----|----|
    |输入条件规定了输入值的范围|等于边界值的数据，刚刚超过边界值的数据|
    |输入条件规定了值的个数|最大个数、最小个数、比最大个数多1、比最小个数少1的数|
    |输入域或输出域是有序集合|选取集合的第1个元素、典型元素、最后1个元素|
  - 边界值分析与等价类分析的区别
    - 边界值分析：着重边界的测试，测试数据：等于、刚刚大于或刚刚小于边界的值
    - 等价类划方：选取测试数据：等价类中的典型值或任意值。
- 因果图
  - *重检查各种输入条件的组合！*
...
id1[分析软件规格说明书，找出一个模块的*原因和结果*，给每个原因和结果*赋予一个標识符*。]
id2[分析原因与结果间、原因与原因间对应关系，*画出因果图*。]
id3[在因果图上*标识*出特定的约束或限制条件。]
id4[把因果图*转换*成*判定表*，]
id5[把判定表的*每一列*拿出来作为依据，*设计测试用例*。]
...
- 黑盒测试技术
  - 概念：将被测软件看作盒子，只通过外部输入、输出发现软件错误，完全不考虑程序内部结构。
  - 别称：功能测试技术
  - 典型：事务处理流程技术」状态测试技术、定义域测试技术
  - 包括：事务流测试、等价类划分、边界值分析等
  - 依据：软件行为的描述

- 在路径测试技术中，*路径选取*是测试用例设计的基础，好的用例设计是发现程序错误的关键。
- 白盒测试技术，又称为*结构测试技术，*典型的是*路径测试技术*。
...
- 单元测试
  ...
  - 指导：详细设计文档。
  - 技术：白盒测试技术。
- 考虑的*模块特征*：*模块接口；局部数据结构；重要的执行路径；错误的执行路径*。
- 测试设备：
  - 测试中，模块不是独立程序，须为每个模块单元测试开发*驱动*模块和*承接*模块。
  - 驱动模块：模拟“主程序〞接受测试用例的数据，将这些数据传送给要测试的模块并打印结果。
  - 承接模块：代替被测模块的下属模块，打印入口检查信息，并将控制返回到它的上级模块。
  - 注意：被测模块的内聚程度高/呈功能性，可简化单元测试
- 集成测试
...
- 单元测试以*详细设计*文档为指导。一般来说，它往往采用*白盒*测试技术。
- 软件产品在交付使用前一般要经过*单元测试*，、集成测试、确认测试和系统测试。
- 在单元测试中，由于模块不是一个独立的程序，必须为每个模块单元测试开发*驱动模块*和*承接模块*，后者代替被测模块的下属模块，打印入口检查信息，并将控制返回到它的上级模块。
- 单元测试期间，通常考虑模块的*错误执行路径*