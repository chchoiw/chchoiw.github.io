...
## 4.2 UML的模型表达格式
- 常用4种建模工具：*类图(結構圖)、用况图（行為圖）、状态图（行為圖）、顺序图（行為圖）*

### 類圖

- 类图是可视化地表达系统*静态结构模型*的工具，通常包含类、* 接口、关联、泛化和依赖关系*等。
- 创建类图的步骤：
  - 模型化待建系统的概念，形成类图中的基本元素；
  - 模型化待建系统的各种关系，形成该系统的初始类图；
  - 模型化系统的协作，给出该系统的最终类图：
  - 模型化逻辑数據库模式。

### 用況圖
- 用况图是一种表达*系统功能模型*的图形化工具。
- 一个用况图通常包含6个模型元素：*主题（Subject ）、用况(Use cases ) 参与者（Actor)、关联、泛化、依赖*。
  - 主题(Subject）由一组用况所描述的一个类，通常是一个系统或子系统。
  - 参与者（Actor）表达了一组高内聚的角色，当用户与用况交互时，该用户扮演这组角色。
  - 关联：一种参与关系，即参与者参与一个用况。
  - 用况之间的3种关系：泛化、扩展、包含。
  - 泛化：用况A和用况B之间具有*一般/特殊*关系。
  - 依赖的变体：包含(<<include>>）、扩展(<<extend >>）
  - 包含：用况A的一个实例*包含*用况B所规约的行为。
  - 扩展：一个用况A的实例在特定的条件下可以由另一用况B所规约的行为予以扩展，并依据定义的扩展点位置，B的行为被插入到A的实例中。
- 用况图的作用：
  - 为系统建模，描述软件系统行为的功能结构；
  - 对业务建模，描述企业或组织的业务过程结构。
  - 业务模型和系统模型之间是“*整体/部分*”关系， 
  - 业务模型是整体，而系统模型是业务模型的一个组成部分。
- 简用况图及其通常包含的模型元素。
  - 用况图是一种表达*系统功能模型*的图形化工具。一个用况图通常包含6个模型元素：
    - *主题（Subject）、用况(Use cases ）、参与者（Actor）、关联、泛化、依赖*。
  
### 狀態圖：

- 状态图是显示一个*状态机*的图，强调了*从一个状态到另一状态*的控制流。
- 状态机：一种行为，规约了一个对象在其生存期间因响应事件并作出响应而经历的状态。
- 状态：类目的一个实例在其生存中的一种条件(condition)或情况(situation)，期间该实例满足这一条件，执行某一活动或等待某一消息。
  - UML把状态分为3类：初态（•）、终态（◎）和正常状态（圆角矩形）
- 事件：对确定的时空内一个有意义发生的规约。在状态机的语境下，一个事件意味着有一个会引发状态转移的激励。
  - 內部事件： 指系统内对象之间传送的事件，例如溢出异常
  - 外部事生指系统和它的参与者之间所传送的事件，如按一下按钮，或传感器的一个中断。
  - 信号 Signal ) 事件
    - 信号：消息的一个类目，是一个消息类型。像类一样，信号可以有属性（以参数形式出现）、操作和泛化。
  - 调用（Call)事件：
    - 调用表示对象接受到一个操作的请求。
  - 时间事件
    - 表示推移一段时间的事件。
    - 通过时间表达式来规约的，例如after 2 seconds, at (1 jan 2007, 12.00).
  - 变化事件
    - 表示状态的一个变化，或表示某一条件得到满足。
  - 状态转移（一>）
    - 两个状态间的一种关系，指一个对象在一个状态中将执行一些确定的动作，当规约的事件发生和规约的条件满足时，进入第二个状态。

### 顺序图
- 顺序图:是一种*交互*图，由一组对象及按时序组织的对象间关系组成， 包含对象间发送的消息。


# 第五章 RUP
- RUP ( Rational Unified Process） 的突出特点：以一种*用况（Use Case） 为驱动*的、以*体系结构为中心的迭代增量式开发*。
-RUP的特点
  - 用况为驱动:
    - 指在系统的生存周期中，*以用况作为基础*，驱动系统有关人员对要建立系统的功能需求进行交流，* 驱动系统分析、设计、实现和测试*等活动。
...
### 需求获取
  - 工具：采用*Use Case技术*获取需求。
  - 目标：使用UML中的用况、參与者及依赖等术语*抽象客观实际问题*，形成系统*的需求获职植型*一 -种特定的*系统/产品模型*，并产生该模型视角下的体系结构描述。

- RUP开展的工作

  |要做的工作| 产生的制品|描述|
  |----|----|----|
  |NO.1 列出候选需求|特征（Feature)列表|先从客户、用户、计划者、开发者的想*法和意愿*中搜取特征，形成特征表。<br>*特征*是一个*新的项*（Item）及其*简要描述*。|
  |NO.2 理解系统语境|领域模型|一般用类图表达，用于捕获系统语境中的一些重要领域对象类。<br>*业务对象*：表示那些由业务操纵的事物，如订单、账目和合同等。<br>|实在对象(Real-world Objects)：如教室、运动场地等。<br>事件（Events)：如上课铃响、讨论开始等。|
  |NO.2 理解系统语境|业务模型|一般用用况图表达,为捕获业务处理和其中的业务对象，通过以下2个层次来抽象一个业务。<br为精化业务用况模型中的每一个业务用况表达业务对象的3个术语：*工作人员、业务实体、工作单元*。|  
  |NO.3 捕获功能需求|用况模型｜目标：*创建系统的用况模型*，以表达客户认可的需求一系统必须满足的条件和能力，作为客户和开发人员间的一种共识 <br>用况模型：是系统的一种*概念模型*，是对系统*功能的抽象*，包括*系统参与者系统用况以及它们之间的关系*。
  |NO.4 捕获非功能需求|补充需求/针对一些特定需求的用况||

- 在RUP的每次迭代中都要经历一个*核心工作流*，即*需求获取、分析、设计、实现和测试*。
  
### 需求分析:
- 目标：在系统*用况模型的基础上*，创建*系统分析模型*及在该分析模型视角下的*体系结构描述*。
- 分析模型：是系统的一种*概念楼型*，解决系统用况模型中,存在的二义性和不一致性等问题，以一种系统化的形式准确表达用户需求。
- *分析类*：类的一种衍型，很少有操作和特征标记，用责任定义其行为，其属性和关系也是概念性的。
  - 边界类：封装一些重要的通信接口和用户界面机制
  - 实体类：封装问题域中的一个重要现象
  - 控制类：封装一些重要的定序
- *用况細化*：一个*协作*。针对一个用况，其行为用*多个分析类间相互作用来细化*，记为用况细[分析」
- *分析包*：一种*控制信息组织复杂性的机制*，提供分析制品的一种组织手段，形成可管理的部分
  - 分析包特徵
    - 体现问题分离；
    - 高内聚、低耦合：
    - 尽可能体现一个系统的完整顶层设计
- 分析模型的表达
- 分析模型由“*分析系统*”定义，该分析系统包含*一组具有层次结构的包*，每个包中可包含一*些分析类和用况细化*[分析]；
- 分析类和用况细化[分析]可单独出现在分析模型中，以凸显其在系统体系结构方面的作用。
- 分析的主要活动
- 创建系统的分析模型，一般应进行4项活动：*体系结构分析、用况分析、类的分析、包的分析*。
- 体系结构分析
  - 目标：
    - 通过标识分析包和分析类，建立分析模型和体系结构“骨架” 标识有关分析包和分析类的特定需求。
- 用况分析
  - 目标：
...
|用况模型|分析模型|
|----|----|
|客户和开发者间关于 “系统应做什么、不应做什儿"的现约|开发者理解系统如何勾固、如何设计、如何实现的基础|
|需求间存在冗余、不一致、冲突等|需求间不应存在冗余、不一致、冲突等|
|捕获系统功能，包括在体系结构方面有意义的功能|给出细化的系统功能，包括在体系结构万面有意义的功能|
...
|輸入|活动|执行者|翰出|
|设计模型、部署模型、体系结构描述 [设计模型、部署模型角度]|实现体系结构|体系结构设计者|构件概述]、体系结构、 描述[实现模型、部署模型角度]|
